...
import type { Voucher, Category, Merchant } from '../../types'

export function AddVoucher() {
  const { data: vouchers, create, update } = useCRUD<Voucher>({ table: 'vouchers' })
  ...
  useEffect(() => {
    if (editId && vouchers.length > 0) {
      ...
      if (v) {
        setForm({
          category: 'All', merchant_id: (v as any).merchant_id || '', issue_based_on: 'reseller',
          code: v.code, generate_type: v.discount_type === 'fixed' ? 'fixed' : 'discount',
          discount_value: String(v.discount_value), description: v.description || '',
          repeat: v.max_uses === 1 ? 'one_time' : 'repeat',
          valid_until: v.valid_until?.split('T')[0] || '', is_active: String(v.is_active), members_only: 'no',
        })
      }
    }
  }, [editId, vouchers])
  ...
  return (
    <div>
      <h1 className="mb-4 text-xl font-bold text-red-700">Add/Edit Voucher</h1>
        ...
        <form onSubmit={handleSubmit}>
          <table className="w-full max-w-xl text-sm" style={{tableLayout: 'fixed'}}><colgroup><col style={{width: '180px'}} /><col /></colgroup>
            <tbody>
              ...
              <tr className="border-b border-gray-100">
                <td className="py-2 pr-4 font-medium text-gray-700 whitespace-nowrap">List/Merchant:</td>
                <td className="py-2">
              ...
              <tr className="border-b border-gray-100">
                <td className="py-2 pr-4 font-medium text-gray-700 whitespace-nowrap">Issue Voucher Based On:</td>
                <td className="py-2">
          ...
          <div className="mt-5 flex gap-2">
            ...
            <Button variant="secondary" type="button" onClick={() => navigate('/admin/vouchers')}>Cancel</Button>
          </div>
        </form>
      </div>
    </div>
